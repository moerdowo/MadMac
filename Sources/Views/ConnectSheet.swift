    ...
    var body: some View {
        ZStack {
            Color(hex: 0x0E0F1A).opacity(0.45)
                .ignoresSafeArea()
                .onTapGesture { if !busy { state.connectOpen = false } }
            ...
            VStack(spacing: 0) {
                ...
                VStack(alignment: .leading, spacing: 16) {
                    field("Access token", hint: "System User token with ads_management + read_insights") {
                    ...
                    (Text("Get a token from Business Manager → System Users, or the Graph API Explorer. The first connection installs Meta's ")
                     + Text("meta-ads").font(.system(size: 12, design: .monospaced))
                     + Text(" CLI into a private environment (needs Python 3.12+)."))
                        .font(jakarta(12))
                ...
                HStack {
                    Btn(variant: .ghost, label: "Use sample data") {
    ...
    private func connect() {
        ...
        Task {
            do {
                ...
                status = "Verifying credentials with `meta ads adaccount list`…"
                _ = try await Sidecar.shared.meta(["ads", "adaccount", "list", "--format", "json"],
                                                  credentials: creds)
                try creds.save()
                status = nil
                busy = false
